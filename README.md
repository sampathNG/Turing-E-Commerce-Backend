...
I have also used JWT-authentication token to verify the if the customer is valid or not. We are already given the mysql-database in which there are different tables and their data. We've to write different queries for different endpoints to make them working.i have use to mysql.
...
Next, navigate to the folder where, `index.js`file is located. You can start the local server by writing`node index.js`on the terminal.
